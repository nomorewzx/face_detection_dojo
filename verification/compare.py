...
def compare(model_path, to_recognize_image_file, face_img_size, margin):
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            face_to_recognize_embeddings = sess.run(embedding_op, feed_dict={imgs_placeholder: faces_to_identify,
                                                                             phase_train_placeholder: False})

            for i in range(face_to_recognize_embeddings.shape[0]):
                dist = np.sqrt(
                    np.sum(np.square(np.subtract(face_to_recognize_embeddings[i, :], face_embeddings)), axis=1))
